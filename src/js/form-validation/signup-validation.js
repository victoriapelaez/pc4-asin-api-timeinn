...
const getUserData = function () {//recogo los datos de los inputs
...
export const signupBtnHandler = function (registerFunction, usersData) {
  if (
    !signupBtn ||
    !usernameInput ||
    !nameInput ||
  ...
  signupBtn.addEventListener('click', e => {
    e.preventDefault();
    e.stopPropagation();
    const data = getUserData();//meto los datos de los inputs de esta funcion en una variable
    if (data.email) {
      usersData.push(data);
      helper.setLocalStorage(usersData);
      helper.toggleAlertVisibility(overlay, alertMsg);
      registerFunction(data);//llamo a la que sera la funcion  con lso datos del fetch
      setTimeout(() => {
        helper.toggleAlertVisibility(overlay, alertMsg);
        setTimeout(() => {
